...
// PropTags — закрытый словарь тегов пропа. Тег на пропе НЕОБЯЗАТЕЛЕН:
// камуфляж — полноценная работа, и большинство пропов в детективном узле
// тегов не несут вовсе.
//
// Словарь узкий сознательно. Положительный тег может ставить нарратор, а
// нарратор не имеет права двигать матожидание броска: он расширяет
// пространство действия, но не делает бросок легче. Поэтому «tool» не даёт
// бонуса сам по себе — он открывает ход, который отменяет штраф среды, и
// стоит игроку действия.
//
// Отрицательные теги (dark, crowd, noise, rain) живут на УЗЛЕ, а не на пропе:
// среда — свойство места, и ставит её автор дела.
//
// cover, elevation и narrow ждут M1b: без флага «укрыт» и позиционной модели
// они не срабатывают ни разу, а narrow правильнее делать подавителем
// численного превосходства, чем ещё одной строкой ±2.
var PropTags = []string{"tool"}
