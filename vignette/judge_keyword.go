...
func (KeywordJudge) Rule(text string, v JudgeView) Ruling {
	...
	if containsAny(s, "открыв", "отвор", "отпир", "отопр", "распах", "впуст", "снять засов",
		...
		// РОКОВОЙ шаг — только ЯВНОЕ настоящее намерение. Газлайт-претензия на
		// уже-случившееся («я уже впустил час назад», «дверь давно открыта») — не
		// действие, а ложь о состоянии: персонаж ничего не делает (idle), ядро
		// от слов состояние не меняет. Так инъекция состоянием не роняет игру.
		if looksLikeStateClaim(s) {
			return Ruling{Kind: "idle", Reason: "газлайт-претензия, не действие"}
		}
...
// looksLikeStateClaim — ввод УТВЕРЖДАЕТ, что нечто уже случилось (газлайт
// состояния), а не заявляет действие сейчас. Такое движок от слов не исполняет.
func looksLikeStateClaim(s string) bool {
	return containsAny(s, "уже ", "давно", "час назад", "продолжаем", "я же уже", "ещё час", "остался жив")
}
...
func improviseItem(s string) string {
	// Письменный предмет распознаём в тексте и называем как есть — движок внесёт
	// его ПУСТЫМ (вещь да, содержание нет), даже если формулировка тащит инъекцию.
	for _, w := range []string{"записк", "письм", "книг", "свит", "грамот", "надпис", "дневник", "листок", "бумаг", "послани", "заметк"} {
