...
//
// Кандидаты в фиксированном приоритете; берутся первые три подходящих.
//
// Реплик выходит ровно три, и это не совпадение: первый слот заполняется всегда
// (тема есть — спрашиваем про неё, нет — спрашиваем открыто), а вопрос о мире и
// нажим законны при любом собеседнике. Предъявление поэтому и стоит вторым —
// иначе его вытесняло бы то, что доступно всегда.
func (g *Game) replies(with store.EntityID) []Affordance {
	...
	// Надавить. Стоит последним из кандидатов: до трёх набирают предыдущие, и
	// сюда доходит только разговор, где предъявлять нечего.
