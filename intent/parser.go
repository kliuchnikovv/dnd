...
3. Спросить можно только о теме из списка известных (question, ask_about). Но
   вопрос о том, чего парти не знает, — не отказ: если игрок обращается к
   человеку, это say с его же словами. Пусть отвечает персонаж — уклонится,
   намекнёт или промолчит. Словарь тут ни при чём.
...
Ответ: {"outcome":"intent","verb":"say","target":"e_ivar","text":"кто убийца?"}

Разговор идёт с e_bern
Игрок: «есть ли какие-нибудь слухи в последнее время?»
Ответ: {"outcome":"intent","verb":"say","target":"e_bern","text":"есть ли какие-нибудь слухи в последнее время?"}
...
func (p *Parser) validate(raw reply, hint SceneHint, text string) (Result, string) {
	...
	if need.Topic && raw.Topic == "" {
		if id, ok := resolveByName(text, hint.Topics); ok {
			raw.Topic = id
		}
	}
	// Открытый вопрос собеседнику темы не имеет и иметь не может: «есть ли
	// слухи?» — это приглашение говорить, а не запрос к банку знаний.
	// Спрашивать в ответ «о чём именно?» значит требовать от игрока знать
	// внутреннее устройство игры. Отвечает персонаж; гейт фактов при этом не
	// трогается — say ничего не выдаёт.
	if need.Topic && raw.Topic == "" && hint.Talk.With != "" &&
		hint.hasEntity(string(hint.Talk.With)) {
		raw.Verb = "say"
		raw.Target = string(hint.Talk.With)
		raw.Text = strings.TrimSpace(text)
		def, _ = core.LookupVerb(raw.Verb)
		need = requires(def.Verb)
	}
