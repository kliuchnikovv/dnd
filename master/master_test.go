...
func TestReplyCarriesSpeakerCard(t *testing.T) {
	...
	// Живой прогон: разрешённый «штрих телом» модель превратила в отдельный
	// абзац третьим лицом («Взгляд скользит поверх плеча…»), и весь этот
	// абзац упал в бабл NPC — сервер обрамление Мастера на разговорном ходу
	// не рисует. Система обязана требовать один абзац и первое лицо.
	sys := f.Calls()[0].System
	if !strings.Contains(sys, "ОДНИМ абзацем") {
		t.Errorf("система реплики не требует одного абзаца:\n%s", sys)
	}
	if !strings.Contains(sys, "первого лица") {
		t.Errorf("система реплики не запрещает третье лицо в штрихе:\n%s", sys)
	}
}
