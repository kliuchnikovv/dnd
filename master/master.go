...
const replySystem = `Ты озвучиваешь персонажа игры: говоришь его прямой речью, от первого лица,
в ответ игроку. Ты — этот персонаж, не рассказчик.

Тебе дана авторская рамка — она ГЛАВНАЯ: она задаёт, что персонаж сейчас
говорит по сути. Ты одеваешь это в его живую речь, не меняя сути.

  - только слова персонажа: без описания его действий, тона, мимики и без
    ремарок вроде «он усмехнулся» — одна реплика, как в кавычках, но без них;
  - одна-две фразы, в его характере и в его положении;
  - обращение к игроку на «вы», если персонаж не из тех, кто «тыкает»;
  - про ДЕЛО ничего сверх рамки: ни имён, ни улик, ни признаний, которых в
    рамке нет. Персонаж вправе уходить от ответа, но не выдавать новых фактов;
  - никаких чисел, бросков и служебных пометок.
...
const (
	...
	// KindReply — прямая речь NPC в ответ на обращение игрока. Отдельно от
	// исхода: исход рисует обрамление (обстановку, что сделал игрок) и НЕ
	// озвучивает персонажа, а реплика — это сами слова персонажа от первого
	// лица. Две части одного разговорного хода: сперва обрамление, затем ответ.
...
func (m *Master) narrateRequest(kind Kind, frame string, w World,
	...
	case kind == KindProbe:
		b.WriteString("Это СВОБОДНАЯ ПРОБА: игрок попробовал что-то, на что в игре нет " +
			...
			"и остался таким же.\n\n")
	case kind == KindReply:
		who := strings.TrimSpace(speaking)
		if who == "" {
			who = "персонаж"
		}
		b.WriteString("Это РЕПЛИКА: ответь игроку прямой речью — ты " + who + ". " +
			"Только его слова, от первого лица; обстановку и его действия НЕ описывай " +
			"(их уже показал исход).\n\n")
	default:
		b.WriteString("Это ИСХОД только что сделанного: покажи, чем оно кончилось. " +
			"Обстановку не пересказывай — игрок её уже видел.\n\n")
	}
	if kind != KindReply && strings.TrimSpace(speaking) != "" {
