...
//
// Speaker — карточка отвечающего NPC, читается только под KindReply. Живёт
// здесь, а не отдельным аргументом, чтобы не разгонять сигнатуру Narrate до
// девяти позиционных полей; для остальных видов прозы поле nil и молчит.
type World struct {
	Setting string
	Scene   []string
	Speaker *Speaker
...
// Speaker — карточка NPC для реплики: голос, быт, отношение к парти,
// незакрытое, желания, память разговора. Заземляет реплику на характер и
// положение персонажа, а не на одну авторскую строку small-talk (талк_ту),
// из-за которой все NPC начинают говорить одним голосом про погоду.
//
// Правды дела здесь нет и быть не может: KnowsAbout/TalksAbout остаются на
// стороне ядра и вплывают в реплику только через авторскую рамку (question.*,
// persuade.* и т.п.). Карточка — про человека, рамка — про дело.
type Speaker struct {
	// Name — как его зовут; Kind — кто он (стражник, писарь, кузнец).
	Name string
	Kind string
	// Voice — авторская затравка речи из дневника.
	Voice string
	// Life — быт: смена, привычки, о чём ворчит. Точки, за которые разговор
	// цепляется, не выдумывая.
	Life string
	// Disposition — расположение к парти. Печатается словом, не числом:
	// «настороженно», «ровно», «тепло» — модели проще одеть в интонацию.
	Disposition int
	// OpenThreads — незакрытое с парти: обещания, долги, угрозы.
	OpenThreads []string
	// Wants — чего персонаж хочет от парти. Разговор — это ещё и его повод
	// попросить, а не только ответить.
	Wants []string
	// TalksAbout — безобидные темы, которые персонаж поднимает сам: авторское
	// топливо светского разговора. Без него на short-talk-рамке про «поговорить
	// без нажима» у него нет тем, кроме той единственной, что в самой рамке —
	// и все реплики сваливаются в неё (у моряка — в погоду, у писаря — в
	// бумаги). Правды дела здесь нет: это авторская проза, не FactID.
	TalksAbout []string
	// Recent — последние круги разговора с этой парти, в порядке произнесения.
	// Против повторов темы («опять о погоде») и против амнезии между ходами.
	Recent []store.Exchange
	// Summary — свёрнутые впечатления от прошлых встреч. Проза, не истина.
	Summary string
}

// render печатает карточку в промпт KindReply. Пустая (nil Speaker или все
// поля пусты) молча пропускает секцию — правило проверяется на месте вызова,
// не тут.
func (s *Speaker) render(b *strings.Builder) {
	if s == nil {
		return
	}
	b.WriteString("Ты — ")
	...
	if v := strings.TrimSpace(s.Voice); v != "" {
		b.WriteString("Голос: " + v + "\n")
	}
	if l := strings.TrimSpace(s.Life); l != "" {
		b.WriteString("Быт: " + l + "\n")
	}
	b.WriteString("Расположение к собеседникам: " + dispositionWord(s.Disposition) + ".\n")
	if len(s.OpenThreads) > 0 {
		b.WriteString("Что между вами не закрыто (это ФОН характера, не повестка каждой реплики — не тащи это в ответ, если игрок сам не касается):\n")
		for _, t := range s.OpenThreads {
			b.WriteString("  — " + t + "\n")
		}
	}
	if len(s.Wants) > 0 {
		b.WriteString("Чего хочет от них (тот же фон — не подставляй вместо ответа):\n")
		for _, w := range s.Wants {
			b.WriteString("  — " + w + "\n")
		}
	}
	if len(s.TalksAbout) > 0 {
		b.WriteString("Запасные темы (используй, только если игрок ни о чём конкретно не спрашивает и надо чем-то заполнить разговор; повторно ту же тему не поднимай):\n")
		for _, t := range s.TalksAbout {
			b.WriteString("  — " + t + "\n")
		}
	}
	if len(s.Recent) > 0 {
		b.WriteString("Последние круги разговора (не повторяйся дословно и не топчись на той же теме):\n")
		for _, e := range s.Recent {
			p := strings.TrimSpace(e.Player)
			r := strings.TrimSpace(e.Reply)
			switch {
			case p != "" && r != "":
				b.WriteString("  — они: «" + p + "» — ты: «" + r + "»\n")
			case r != "":
				b.WriteString("  — ты: «" + r + "»\n")
			case p != "":
				b.WriteString("  — они: «" + p + "»\n")
	...
	if sm := strings.TrimSpace(s.Summary); sm != "" {
		b.WriteString("Впечатления от прошлых встреч: " + sm + "\n")
	}
}

// dispositionWord — расположение словом. Число сюда просачивалось бы служебной
// пометкой, а промпт про интонацию, не про баланс.
func dispositionWord(d int) string {
	...
	case d <= -1:
		return "настороженно"
	case d == 0:
		return "ровно, без тепла"
	case d <= 2:
		return "приветливо"
	default:
		return "тепло, по-свойски"
	}
}
...
говорит по сути. Ты одеваешь это в его живую речь, не меняя сути. Если рядом
приведена карточка персонажа (голос, быт, расположение, незакрытое, желания,
память разговора) — говори ИЗ НЕЁ: она про человека, не про дело.

  - ОТВЕЧАЙ НА ТО, ЧТО СПРОСИЛ ИГРОК. Если он спросил про мир (устройство,
    погоду, работу, быт, слухи) — говори про мир. Если он просто поболтал —
    поддержи разговор. Не переводи каждую реплику на дело, на бумаги, на
    невыполненные обещания: у игрока должно быть право «просто поговорить».
    Незакрытое из карточки — это ФОН характера, не повестка каждой реплики;
    оно всплывает само, только если игрок его прямо касается, или изредка —
    как одна короткая ремарка в конце, не как встречный вопрос;
  - запасные темы из карточки (talks_about) поднимай, только если игрок ни о
    чём конкретно не спросил и разговор надо чем-то заполнить. Если игрок
    задал прямой вопрос — сперва ответ, темы карточки не вместо ответа;
  - ОДНИМ абзацем, без пустых строк и без переносов между ремарками и речью:
    твоя реплика — это одна короткая реплика, не сцена;
  - слова персонажа. Штрих телом допустим один — коротким тире внутри речи, от
    первого лица («— Погода, что тут сказать, — я поправил плащ, — третью
    неделю льёт»). Не отдельный абзац-описание («Взгляд скользит поверх
    плеча…»), не «он усмехнулся» третьим лицом, не пересказ внутреннего мира.
    Обстановку не пиши: её уже показал исход, а на разговорном ходу обрамления
    Мастера не будет — всё, что ты выдашь, попадёт в бабл персонажа;
  ...
  - НЕ повторяй тему последнего круга и не начинай реплику теми же словами,
    что раньше: у карточки есть память разговора — сверься с ней. Если сказать
    больше нечего, персонаж вправе перевести разговор или коротко отшутиться;
...
const (
	...
	// KindClarify — неясное обращение: из слов игрока непонятно, к кому он или
	// что именно делает. Вместо служебного «уточни» мир отвечает диегетически:
	// слова повисли, на игрока смотрят непонимающе. Ход не состоялся, ничего не
	// разрешается — только просьба сказать яснее, вплетённая в сцену.
...
func (m *Master) narrateRequest(kind Kind, frame string, w World,
	...
	case kind == KindPlace:
		b.WriteString("Это ОПИСАНИЕ МЕСТА: покажи, что игрок видит вокруг. Коротко — " +
			"две-три фразы, один абзац. Выдели то, за что можно зацепиться: кто здесь, " +
			"что на виду, куда можно пойти. Погоду и настроение — одним штрихом, не " +
			"перечнем; не растекайся по атмосфере.\n\n")
	case kind == KindProbe:
	...
	case kind == KindReply:
		...
		b.WriteString("Это РЕПЛИКА: ответь игроку прямой речью — ты " + who + ". " +
			"Слова от первого лица; обстановку не описывай (её уже показал исход). " +
			"Один короткий штрих телом (взгляд, жест) допустим — не больше.\n\n")
		if w.Speaker != nil {
			w.Speaker.render(&b)
			b.WriteString("\n")
		}
	case kind == KindClarify:
		b.WriteString("Это НЕЯСНОЕ ОБРАЩЕНИЕ: из слов игрока непонятно, к кому он " +
			"обращается или что именно делает. Покажи это в мире — его слова повисли " +
			"без ответа, на него смотрят непонимающе и ждут, пока он скажет яснее " +
			"(к кому и что). Ничего НЕ разрешай, не открывай и не двигай; о деле ни " +
			"слова. Одна-две фразы.\n\n")
	default:
