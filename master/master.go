...
func (s *Speaker) render(b *strings.Builder) {
	...
	if len(s.OpenThreads) > 0 {
		b.WriteString("Незакрытое с ними: " + strings.Join(s.OpenThreads, "; ") + ".\n")
	}
	if len(s.Wants) > 0 {
		b.WriteString("Чего хочет от них: " + strings.Join(s.Wants, "; ") + ".\n")
	}
	if len(s.TalksAbout) > 0 {
		b.WriteString("О чём охотно заговаривает сам (выбери одно, если по рамке уместно; повторно ту же тему не поднимай):\n")
