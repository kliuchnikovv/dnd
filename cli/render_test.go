...
// она. Второй плейтест из этого вывел, что обвинение требует трёх независимых
// источников, и остановился, имея на руках всё нужное: подтверждение делает
// дело крепким, но обвинять можно и без него.
func TestFactsLegendDoesNotImplyAGate(t *testing.T) {
	g := renderGame(t)
	got := Render{}.Facts(g)
