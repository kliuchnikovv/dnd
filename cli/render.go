...
const (
	...
	// ProseProbe — отклик на свободную пробу. Не исход: исхода не было, ядро
	// хода не считало, и описывать «чем оно кончилось» нечего. Мастер
	// показывает, что игрок при этом увидел и почувствовал, — и ничего больше.
	ProseProbe ProseKind = "probe"
	// ProseReply — прямая речь NPC в ответ на обращение игрока. Идёт после
	// обрамляющего исхода: исход показал обстановку, реплика — сами слова.
	ProseReply ProseKind = "reply"
	// ProseClarify — диегетический отклик на неясное обращение: мир не понял, к
	// кому/о чём игрок, и ждёт, пока скажет яснее. Ход не состоялся.
	ProseClarify ProseKind = "clarify"
)
...
type Prose struct {
	...
	Outcome []string
	// Probe — что игрок пробует, когда словарь такого не выражает. Только у
	// ProseProbe: у остальных видов ход заявлен глаголом, и пробы нет.
	Probe string
...
// Probe — отклик на свободную пробу.
//
// Рамка кодовая, а не авторская, и это выбор. У Мастера пустая рамка означает
// молчание — описывать нечего, — а молчание игрок читает как поломку. Завести
// же под пробу ключ дела значило бы завести дыру: незаполненный ключ печатается
// как [probe.n_quay] и живёт так до самого демо. Своеобразие отклика несут
// сцена и слова самой пробы, а не авторская строка, которой ни в одном деле
// пока нет.
...
func (Render) Help() string {
	return `1..4                            выбрать вариант из списка «Что можно»
survey                          что здесь можно трогать
