...
var actWords = []struct {
	...
	{ActFarewell, []string{"прощай", "до свидания", "пока", "всего доброго",
		"до встречи", "бывай", "увидимся", "счастливо", "откланяюсь",
...
//
// Считается НЕ по длине реплики. Модель с рассуждением тратит вывод раньше,
// чем доходит до текста, и потолок «на одну-две фразы» обрезает ответ до
// пустой строки: живой прогон отдал ровно 200 токенов и ничего не сказал.
// Экономия здесь мнимая — обрезанный ответ оплачен полностью и выброшен.
